...
export default function FreightPopup({open, onClose, onApply, initialExpenses = [], readOnly = false}) {

  const dispatch = useDispatch();
  const { freights } = useSelector((state) => state.freight);

  const [rows, setRows] = useState([createRow(1)]);

  useEffect(() => {
    if (!open) return;
    if (freights.length === 0) dispatch(getFreights());
  }, [open]);
    ...
    if (initialExpenses.length > 0) {
      setRows([
        ...initialExpenses.map((exp, i) => {
          if ('freightCode' in exp) return { ...exp, id: i + 1 };
          const freightName =
            freights.find((f) => String(f.ExpensCode) === String(exp.ExpenseCode))?.Name ||
            exp.ExpenseCode?.toString() || '';
          const taxPct = exp.TaxPercent ?? exp.TaxPercentagePerRow ?? 0;
          const amount = parseFloat(exp.LineTotal || 0);
          const taxAmount = (amount * taxPct) / 100;
          return {
            id: i + 1,
            freightCode: exp.ExpenseCode?.toString() || '',
            freightName,
            remark: exp.Remarks || '',
            amount: exp.LineTotal?.toString() || '',
            taxGroup: exp.VatGroup || '',
            taxPercentage: taxPct ? taxPct.toString() : '',
            taxAmount: taxPct ? taxAmount.toFixed(2) : '',
            grossAmount: (amount + taxAmount).toFixed(2)
          };
        }),
        createRow(initialExpenses.length + 1)
      ]);
    } else {
      setRows([createRow(1)]);
    }
                        ...
                        InputProps={{
                          ...
                          endAdornment: (
                            <InputAdornment position="end">
                              ...
                              >
                                <SearchIcon sx={{ color: readOnly ? 'text.disabled' : '#2196f3', fontSize: 16 }} />
                              </IconButton>
                            </InputAdornment>
                          )
                        }}
        ...
        >
          {readOnly ? (
            ...
            <>
              ...
              <Button variant="contained" color="secondary" onClick={handleApply}>Continue</Button>
            </>
          )}
        </Box>
      </DialogContent>
