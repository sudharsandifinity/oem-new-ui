...
import { mapApiToForm, mapApiToRows } from './ARInvoiceHelpers';
import { getARInvoiceById, resetARInvoiceState } from '../../store/slices/ARInvoiceSlice';
...
export default function SalesInvoicesView() {
  ...
  const { currentInvoice, loading, error } = useSelector((state) => state.ARInvoice);
  ...
  const isLoading = loading || !salesInvoice;
        ...
        <Box sx={{ p: 3 }}>
          {error ? (
            ...
            <>
              <Box sx={{ display: tabValue === 0 ? 'block' : 'none' }}>
                <GeneralTab data={salesInvoice} setData={noop} readOnly />
              </Box>
              <Box sx={{ display: tabValue === 1 ? 'block' : 'none' }}>
                <ContentTab data={salesInvoice} setData={noop} rows={documentLines} setRows={noop} readOnly />
              </Box>
              <Box sx={{ display: tabValue === 2 ? 'block' : 'none' }}>
                <AttachmentTab data={salesInvoice} setData={noop} readOnly />
              </Box>
            </>
          )}
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, flexWrap: 'wrap' }}>
            ...
            <Button
              ...
              onClick={() => navigate(`/A/R-Invoice/edit/${id}`)}
            >
              Edit
            </Button>
          </Box>
