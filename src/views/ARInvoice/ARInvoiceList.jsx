...
import { getARInvoices } from '../../store/slices/ARInvoiceSlice';
...
export default function ARInvoiceList() {
  ...
  useEffect(() => {
    dispatch(getARInvoices());
  }, []);

  const { loading, error, invoices } = useSelector((state) => state.ARInvoice);
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <Stack direction="row" spacing={1}>
          <IconButton size="small" color="primary" onClick={() => navigate(`/A/R-Invoice/view/${ row.original.DocEntry}`)}>
            <VisibilityIcon fontSize="small" />
          </IconButton>

          <IconButton size="small" color="secondary" onClick={() => navigate(`/A/R-Invoice/edit/${ row.original.DocEntry}`)}>
      ...
      >
        ...
        >
          ...
          <Button
            ...
            color="secondary"
            startIcon={<AddIcon />}
            onClick={() => navigate('/A/R-Invoice/create')}
            sx={{
              minWidth: 140,
              whiteSpace: 'nowrap'
            }}
