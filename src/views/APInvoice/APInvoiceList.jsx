...
import { getAPInvoices } from '../../store/slices/APInvoiceSlice';
...
export default function APInvoiceList() {
  ...
  useEffect(() => {
    dispatch(getAPInvoices());
  }, []);

  const { loading, error, invoices } = useSelector((state) => state.APInvoice);
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <Stack direction="row" spacing={1}>
          <IconButton size="small" color="primary" onClick={() => navigate(`/A/P-Invoice/view/${ row.original.DocEntry}`)}>
            <VisibilityIcon fontSize="small" />
          </IconButton>

          <IconButton size="small" color="secondary" onClick={() => navigate(`/A/P-Invoice/edit/${ row.original.DocEntry}`)}>
      ...
      <MainCard content={false} sx={{ mb: 3, flexShrink: 0 }}>
        ...
        >
          ...
          <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} onClick={() => navigate('/')}>
            ...
            <Typography variant="body2">A/R Invoices</Typography>
      ...
      >
        ...
        >
          ...
          <Button
            ...
            color="secondary"
            startIcon={<AddIcon />}
            onClick={() => navigate('/A/P-Invoice/create')}
            sx={{
              minWidth: 140,
              whiteSpace: 'nowrap'
            }}
