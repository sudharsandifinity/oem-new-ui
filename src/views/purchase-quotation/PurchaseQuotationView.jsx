...
import { mapApiToForm, mapApiToRows } from './PurchaseQuotationHelpers';
import { getPurchaseQuotationById, resetPurchaseQuotationState } from '../../store/slices/purchaseQuotationSlice';
...
export default function PurchaseQuotationsView() {
  ...
  const { currentOrder, loading, error } = useSelector((state) => state.PurchaseQuotation);
  ...
  const isLoading = loading || !purchaseQuotation;
        ...
        <Box sx={{ p: 3 }}>
          {error ? (
            ...
            <>
              <Box sx={{ display: tabValue === 0 ? 'block' : 'none' }}>
                <GeneralTab data={purchaseQuotation} setData={noop} readOnly />
              </Box>
              <Box sx={{ display: tabValue === 1 ? 'block' : 'none' }}>
                <ContentTab data={purchaseQuotation} setData={noop} rows={documentLines} setRows={noop} readOnly />
              </Box>
              <Box sx={{ display: tabValue === 2 ? 'block' : 'none' }}>
                <AttachmentTab data={purchaseQuotation} setData={noop} readOnly />
              </Box>
            </>
          )}
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, flexWrap: 'wrap' }}>
            ...
            <Button
              ...
              onClick={() => navigate(`/purchase-quotation/edit/${id}`)}
            >
              Edit
            </Button>
          </Box>
